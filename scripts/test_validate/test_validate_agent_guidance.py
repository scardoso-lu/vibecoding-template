...
from scripts.validate.services.agent_guidance import validate_agent_guidance


def test_detects_stale_agent_guidance_terms(tmp_path: Path) -> None:
    (tmp_path / "AGENTS.md").write_text(
        "Use e2e-explorer and e2e/report.md\n", encoding="utf-8"
    )
...
def test_detects_root_guidance_similarity_drift(tmp_path: Path) -> None:
    ...
    (tmp_path / "CLAUDE.md").write_text(
        "Completely different Claude text " * 40, encoding="utf-8"
    )
