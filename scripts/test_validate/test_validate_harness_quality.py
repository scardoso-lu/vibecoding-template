...
def test_frontend_e2e_only_folder_does_not_require_frontend_app_artifacts(
    tmp_path: Path,
) -> None:
    write(tmp_path / "frontend/e2e/order.spec.ts", "test('contract', async () => {})\n")

    layout_findings = validate_project_layout(tmp_path)
    frontend_findings = validate_frontend_contract(tmp_path)

    assert layout_findings == []
    assert frontend_findings == []
...
def test_qa_evidence_requires_command_provenance(tmp_path: Path) -> None:
    write(
        tmp_path / "memory/feature/inventory/slice.md",
        """## Status
- State: active
## Request
Build app.
...
def test_qa_evidence_rejects_a_run_whose_output_path_does_not_exist(
    tmp_path: Path,
) -> None:
    # scripts/validate/cli.py gate always writes real captured output to each run's
    # output_path (see services/gate.py::run_command). A run entry naming an
    # output_path that was never written is a sign the evidence file was
    # hand-authored rather than produced by the gate runner, which CLAUDE.md
    # forbids ("QA evidence must be ... generated by the gate runner").
...
""",
    ...
    write(
        ...
        json.dumps(
            {
                "generated_by": {
                    "command": "python scripts/validate/cli.py gate --root . --slice memory/feature/inventory/slice.md",
                    "cwd": ".",
                },
                "runs": [
                    {
                        ...
                        "output_path": "memory/feature/inventory/evidence/layout.txt",
                    }
                ],
            }
        ),
    )
    # Deliberately not writing memory/feature/inventory/evidence/layout.txt.

    findings = validate_qa_evidence(tmp_path)
    messages = "\n".join(finding.format() for finding in findings)

    assert "output_path does not exist" in messages
    assert "evidence/layout.txt" in messages


def test_qa_evidence_accepts_a_run_whose_output_path_exists(tmp_path: Path) -> None:
    slice_dir = tmp_path / "memory/feature/inventory"
    write(
        slice_dir / "slice.md",
        """## Status
...
""",
    ...
    write(slice_dir / "evidence/layout.txt", "project-layout: ok\n")

    findings = validate_qa_evidence(tmp_path)
    messages = "\n".join(finding.format() for finding in findings)

    assert "output_path does not exist" not in messages
...
def test_qa_evidence_does_not_require_frontend_app_commands_for_e2e_only_folder(
    tmp_path: Path,
) -> None:
    slice_dir = tmp_path / "memory/feature/inventory"
    write(
...
| Criteria | Test Type | Test Location |
|---|---|---|
| AC-001 | e2e-contract | frontend/e2e/inventory.spec.ts |
## QA Handoff
- review
## Provenance
- source
""",
    ...
    write(tmp_path / "frontend/e2e/inventory.spec.ts", "test('contract', async () => {})\n")
    write(
        slice_dir / "qa-evidence.json",
        json.dumps(
            {
                ...
                "unit_coverage": [
                    {
                        ...
                        "minimum_percent": 80,
                        "actual_percent": 100,
                        "summary_path": "memory/feature/inventory/evidence/docs.json",
                    }
                ],
                "e2e_coverage_path": "memory/feature/inventory/e2e-coverage.json",
            }
        ),
    )
    write(slice_dir / "e2e-coverage.json", "{}")
    write(slice_dir / "evidence/docs.json", "{}")

    findings = validate_qa_evidence(tmp_path)
    messages = "\n".join(finding.format() for finding in findings)

    assert "frontend test command" not in messages
    assert "frontend build command" not in messages
    assert "frontend unit coverage" not in messages
                ...
                "runs": [
                    {
                        ...
                        "started_at": "2026-06-29T00:00:00Z",
                        "finished_at": "2026-06-29T00:00:01Z",
                        "output_path": "memory/feature/inventory/evidence/backend.txt",
                    }
                ],
            }
        ),
        ...
        json.dumps(
            {
                ...
                "runs": [
                    ...
                    },
                    {
                        ...
                        "output_path": "memory/feature/inventory/evidence/docker-compose-up.txt",
                    },
                    {
                        "command": "python scripts\\validate\\cli.py runtime-smoke --config memory\\feature\\inventory\\runtime-smoke.json",
                        "cwd": ".",
                        "exit_code": 0,
                        "started_at": "2026-06-29T00:00:03Z",
                        "finished_at": "2026-06-29T00:00:04Z",
                        "output_path": "memory/feature/inventory/evidence/runtime-smoke.txt",
                    },
                    {
                        "command": "uv run pytest --cov=src --cov-report=json:coverage.json --cov-fail-under=80",
                        "cwd": "backend",
                    ...
                    },
                    {
                        ...
                        "output_path": "memory/feature/inventory/evidence/frontend.txt",
                    },
                    {
                        ...
                        "output_path": "memory/feature/inventory/evidence/build.txt",
                    },
                    {
                        ...
                        "output_path": "memory/feature/inventory/evidence/e2e.txt",
                    },
                    {
                        ...
                        "output_path": "memory/feature/inventory/evidence/docker-compose-down.txt",
                    },
                ],
                "unit_coverage": [
                    {
