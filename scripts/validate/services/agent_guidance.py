...
from scripts.validate.models import Finding, iter_text_files, line_number, read_text
...
def similarity(left: str, right: str) -> float:
    return difflib.SequenceMatcher(
        None, normalize_runtime_text(left), normalize_runtime_text(right)
    ).ratio()
...
def validate_agent_guidance(root: Path) -> list[Finding]:
        ...
        for term, hint in STALE_TERMS.items():
            for match in re.finditer(re.escape(term), text):
                findings.append(
                    Finding(
                        rel,
                        f"stale workflow term {term!r}: {hint}",
    ...
    if (root / "AGENTS.md").exists() and (root / "CLAUDE.md").exists():
        ...
        if score < 0.90:
            findings.append(
                Finding(
                    "AGENTS.md",
                    f"AGENTS.md and CLAUDE.md similarity is {score:.2%}; expected at least 90%",
                )
            )
    ...
    mirror_pairs = [
        ...
        (".claude/agents/qa-checker.md", ".codex/agents/qa-checker.toml"),
        (".claude/agents/qa-challenger.md", ".codex/agents/qa-challenger.toml"),
        (".claude/guideline-routing.md", ".codex/guideline-routing.md"),
        (
            ".claude/templates/template-routing.md",
            ".codex/templates/template-routing.md",
        ),
        (
    ...
    for left, right in mirror_pairs:
        ...
        if not left_path.exists():
            continue
        score = similarity(
            comparable_guidance_text(left_path), comparable_guidance_text(right_path)
        )
        if score < 0.65:
            findings.append(
                Finding(
                    left,
                    f"mirrored artifact similarity with {right} is {score:.2%}; review drift",
                )
            )
    ...
    for left, right in [
        ...
        if (root / left).exists() and (root / right).exists():
            ...
            if "E2E Test Stories" in ltext and "E2E Test Stories" not in rtext:
                findings.append(
                    Finding(
                        right, f"missing E2E Test Stories language mirrored from {left}"
                    )
                )
            if "E2E Test Stories" in rtext and "E2E Test Stories" not in ltext:
                findings.append(
                    Finding(
                        left, f"missing E2E Test Stories language mirrored from {right}"
                    )
                )
