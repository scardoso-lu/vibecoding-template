...
# compaction too, but its stdout is never fed back into context (side-effects only), so it cannot
# do this file's job. track-compact.sh is the PostCompact hook - it only logs that a compaction
# happened, for out-of-band review.
set -uo pipefail
