...
case "$AGENT" in
  backend-developer|frontend-developer|qa|challenger|orchestrator)
    deny "Only the planner may call the guidelines MCP server. Stop and request targeted context through the orchestrator (see the MCP budget rules in AGENTS.md) - do not resolve slugs or browse MCP from a '$AGENT' subagent." ;;
esac
