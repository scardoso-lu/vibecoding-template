...
| `guard-edits.sh` | `PreToolUse` | `Edit\|Write\|apply_patch` | Blocks edits to review-only `feature-memory/history/**` and secrets files (`.env`, `.env.*`; `.env.example` stays editable). Also confines the **e2e-explorer** to writing under `feature-memory/<slice>/e2e/`. |
...
| `reinject-context.sh` | `SessionStart` | `compact` | After compaction, re-injects the 3 AGENTS.md rules + the deterministic-gate model + the active feature-memory slice states. |
...
- **e2e-explorer write scope** (`guard-edits.sh`): when `agent_type` is `e2e-explorer`,
  writes are allowed only under `.codex/feature-memory/<slice>/e2e/`; anything else is
...
- **`reinject-context.sh` (`SessionStart`, matcher `compact`)** — compaction can drop the operating
  rules. Anything it prints to stdout is added back to context, so it restates the three AGENTS.md
...
echo '{"agent_type":"e2e-explorer","tool_input":{"file_path":"src/x.ts"}}' | .codex/hooks/guard-edits.sh
