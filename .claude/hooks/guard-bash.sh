...
# documented Claude Code PreToolUse integration. Adopting it would mean replacing this
# hook-script model with an external policy engine - a much larger architecture change than
# this task, so it was not adopted here. Revisit if a Claude Code-native integration ships.
set -uo pipefail
