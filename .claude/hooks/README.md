...
| `verify-coordination.sh` | `Stop` | - | Deterministic coordination gate: scans only the current turn's assistant output and, when it finds a `## Coordinate Handoff`/`## Route Handoff` declaration or a workflow-agent invocation, blocks once with the coordination checklist (one mode per step, no plan/memory/code writing in place of the owning agent, both 90% challenge gates in the same round, round count + acceptance percentages recorded, Do Not Touch + Stop condition on implementer/QA rows, routing table + Depends-on graph honored) so the main thread self-reviews before finishing. Free on turns with no coordination in them - it replaced an always-on Stop prompt gate that paid a model call every turn (Stop hooks accept no matcher or `if` pre-filter). Paired with `verify-round-cap.sh` below, which recomputes the round count instead of trusting any judgment at all. |
...
- **MCP is software-architect-only** (`guard-mcp.sh`): calls to `mcp__fullstack-guidelines__*`
...
- **Agent infrastructure reads are coordination-tier-only** (`guard-infra-read.sh`): direct
  ...
  `scripts/` are denied for implementer/QA subagents - including pathless `Grep`/`Glob`, whose
  default scope is the repo root. The coordination tier and built-in utility subagents
  (`hook_json_is_builtin_utility_agent()`: Explore, Plan, general-purpose, and the CLI helper
  agents, which run on the main thread's behalf) pass through. The
  Bash guard also blocks obvious shell reads/searches of those paths.
  Whitelist: every subagent may read `.claude/templates/**` and `.claude/skills/**` (and the
  `.codex/` mirrors) - they are reference material (e.g. qa-checker's own prompt points at
  `.claude/templates/categories/e2e.md`), not agent infrastructure.
...
It is invoked through `.claude/hooks/run-hook.py`, which runs on Windows, macOS, and Linux.
The launcher resolves Bash from `PATH`, common Git-for-Windows install locations, or `GIT_BASH`.

The committed hook command is `python .claude/hooks/run-hook.py ...`, so Python must be
...
- **`context-usage-watch.sh` (`PostToolUse`, all tools)** - auto-compaction can hit before anyone
  ...
  `CONTEXT_WINDOW_TOKENS` (default `200000`) it returns `additionalContext` telling Claude to
...
- **`resume-handoff.sh` (`SessionStart`, matcher `startup|resume`)** - the pickup half of the
  ...
  first** whether to inject the handoff; on yes, Claude reads the file, follows its References to
...
- **`verify-round-cap.sh` (`Stop`)** gives the Plan-Loop round count and 3-round cap a real,
  ...
  writes) and counts each business-challenge/technical-challenge occurrence itself: the Nth
  occurrence of a step IS round N, so a declaration that lags the hook's own count (the "stuck at
  Round 1 forever" loop), a count past 3 regardless of what is declared, or a declared round past
  3 all hard-block. A declaration ahead of the count is trusted upward - a cleaned temp dir must
  fail open, not false-block. `verify-coordination.sh` still reviews everything else about the
  loop; this hook only owns the one invariant that can be computed deterministically.
...
directly, reviewed by the `verify-coordination.sh` and `verify-round-cap.sh` `Stop` gates instead of a
...
Hooks read a JSON event on stdin. Simulate one through the same cross-platform launcher Claude uses:

```bash
printf '%s\n' '{"tool_input":{"command":"git status"}}' | python .claude/hooks/run-hook.py .claude/hooks/guard-bash.sh
```
...
echo '{"agent_type":"qa-checker","tool_input":{"file_path":"src/x.ts"}}' | .claude/hooks/guard-edits.sh
