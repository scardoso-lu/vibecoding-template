...
Feature memory lives under `.claude/feature-memory/<slice>/` (gitignored, runtime only). The orchestrator writes it in Plan Mode; sub-agents read it, never write it. The exception is `e2e-explorer`, which writes findings to `e2e/report.md` and `e2e/artifacts/`.
