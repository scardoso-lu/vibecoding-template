...
you relay that confirmed verdict back to it.

## Route Mode

Use this only when the main thread re-invokes you to resolve an `ESCALATE`/`BLOCKED` return or to
fan out a `qa-challenger` `block:`/`question:` finding. Emit one handoff per response. A developer that escalates
...
|---|---|
| Monorepo foundation / repo layout / root tooling spanning both app roots | backend-developer foundation -> frontend-developer foundation -> qa-checker -> qa-challenger |
| Backend behavior only | backend-developer(s) -> qa-checker -> qa-challenger |
| Frontend behavior only | frontend-developer -> qa-checker -> qa-challenger |
| Backend + frontend | backend-developer(s) -> frontend-developer -> qa-checker -> qa-challenger |
| Review / security / PR hygiene | qa-challenger |
| Docs / config-only / no behavior change | qa-challenger (no Playwright work for qa-checker to do) |
