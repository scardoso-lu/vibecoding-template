...
in accepted PRDs, ADRs, fetched guidelines, or explicit product requirements, stop and ask the user
for examples.
...
- One `memory/feature/<feature-slice>/slice.md` per business feature, built as one cohesive,
  homogeneous pack: that feature's structure/scaffolding, backend behavior, frontend behavior, and
  tests belong in the same slice - do not split them into separate slices or a separate foundation
  slice unless the structure is genuinely shared by multiple already-planned, unrelated features.
...
Avoid over-splitting: keep each purpose's technical decisions as one cohesive ADR by default. Split
off a component ADR only when the component is genuinely heterogeneous - independently shippable/
owned, or mixing decisions that different reviewers must approve separately - not because the ADR got
long. Keep parent/component links so downstream agents can read or grep the broader context.
...
Derive feature slices as cohesive, homogeneous packs that cover the accepted PRDs and ADRs - one
slice per business feature, bundling whatever structure, backend, frontend, and test work that
feature needs. Avoid over-splitting into separate slices for what is really one pack of work.
Complete each `slice.md` with the selected category templates, including PRD/ADR/rule dependencies,
contracts, implementation plan, test coverage, QA handoff, and provenance.

For user-facing slices, verify `E2E Test Stories` and `e2e-coverage.json` map initial-prompt
`US-###` stories to Playwright test IDs.

Every slice must carry a `## Verification` section (see `categories/base-slice.md`): one
`- Run: <command> | covers: AC-###` row per verification command, whose `covers:` union spans
every acceptance criterion, with each command naming the mapped test file(s). This is a
machine-parsed contract - the gate runner executes the rows and the `verification` validator
blocks QA on a row that never ran, failed, or went stale. Use the explicit
`- Run: none [skip-verify: <reason>]` escape only for criteria with no runnable surface
(docs-only, or staged per rule 6); never leave the section out or write a vacuous command.
