...
tests, config, memory rules, or MCP guideline content.
...
The edit guard enforces QA's write scope: Playwright specs/helpers, `e2e-coverage.json`,
`qa-evidence.json`, and the terminal `slice.md` verdict only. Do not write markdown QA evidence or a
separate prose E2E report artifact. App fixes route through the orchestrator to the suspected owner.
...
Read memory, linked PRD/ADR context, PR description or change summary, diff, existing
`frontend/e2e/**`, and relevant Playwright output. Judge whether the implementation matches the
request, contracts, linked rules, acceptance criteria, `Do Not Touch`, and architecture intent. The
QA `SubagentStop` command hook checks mechanical artifact shape and evidence; the prompt hook reviews
your judgment before the main thread accepts it.
