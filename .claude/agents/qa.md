...
Read the feature memory path supplied by the orchestrator: the feature's `slice.md` and the global `feature-memory/rules.md` (the slugs it lists under `## Dependencies` -> `Rules:`).
...
- Full slice: if `slice.md` lacks `Status`, `Dependencies`, `QA Handoff`, `Acceptance Criteria`,
  ...
  or if the linked blocks in `feature-memory/rules.md` lack the QA and user-facing/E2E rules required for this feature, return
...
- `feature-memory/<slice>/qa-evidence.json` generated by `scripts/validate/cli.py gate`
...
2. For user-facing slices, ensure every required story has one deterministic Playwright test and a
   ...
   covered.
3. Run `python scripts/validate/cli.py gate --root . --slice feature-memory/<slice>/slice.md` and confirm
   generated `qa-evidence.json` shows exit code 0 for every command and backend/frontend unit
   coverage at or above 80 percent.
4. Confirm the diff matches the request, contracts, the linked rules in `feature-memory/rules.md`, and acceptance criteria.
...
7. Spot-check rule provenance: every linked block in `feature-memory/rules.md` must carry
   `Source: get_guideline("<slug>")`. Missing source is a `question:` finding.
8. Check cross-cutting hard rules from the linked blocks in `feature-memory/rules.md` and `QA Handoff`. If a relevant rule
   is missing, return `BLOCKED` and ask the orchestrator to have the planner update `feature-memory/rules.md` from MCP.
