...
Read the feature memory path supplied by the orchestrator: the feature's `slice.md` and the global `feature-memory/rules/<category>.md` files it references under `## Dependencies` -> `Rules:`.
...
- Full slice: if `slice.md` lacks `Status`, `Dependencies`, `QA Handoff`, `Acceptance Criteria`,
  ...
  or if the referenced `rules/*` files lack the QA and user-facing/E2E slug rules required for this feature, return
...
4. Confirm the diff matches the request, contracts, slugs in the referenced `rules/*` files, and acceptance criteria.
...
7. Spot-check rule provenance: every block in the referenced `rules/*` files must carry
   `Source: get_guideline("<slug>")`. Missing source is a `question:` finding.
8. Check cross-cutting hard rules from the referenced `rules/*` files and `QA Handoff`. If a relevant rule category is
   missing, return `BLOCKED` and ask the orchestrator to have the planner update the rules library from MCP.
