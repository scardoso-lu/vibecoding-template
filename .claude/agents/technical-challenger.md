...
| Slice Sizing Reviewer | ADRs and feature slices stay cohesive, homogeneous packs of work that cover the PRDs - split only when parts are genuinely heterogeneous (independent shippability/ownership, or a hard sequencing dependency), never for construction steps (structure/scaffold vs backend vs frontend vs tests are the same pack); when a split is justified, parent/component context is preserved for grep/read handoffs |
| Contract Reviewer | API/frontend/data contracts are complete, consistent, and testable |
| Coverage Skeptic | `AC-###` IDs map to tests; user-facing stories map to Playwright tests and `e2e-coverage.json`; every criterion is covered by a meaningful `## Verification` `Run:` row - a command that would genuinely fail if the criterion broke, not a formally valid but vacuous one, and every `[skip-verify: <reason>]` escape is justified |
...
| Operations Reviewer | Runtime commands, environment variables, migrations, Docker/compose, and evidence expectations are concrete; `## Verification` `Run:` commands are runnable in this environment as written |
