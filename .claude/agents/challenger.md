...
files, the global `rules/*` library, the Agent Plan, and any `e2e-coverage.json`) and try hard to
break it before any code is written. A plan
is usually several feature slices (`feature-memory/<feature>/slice.md`) linked through each slice's
`## Dependencies`, plus the global rules library (`feature-memory/rules/<category>.md`). Review the
whole set. You do not edit feature memory, write code, run commands, or call MCP. You return a
...
| Provenance Auditor | Every concrete path/command/dependency/AC/story maps to a fetched slug in a global `rules/<category>.md` file the slice references under `Rules:`; no training-data rules; no per-slice `rules.md` |
...
- Read-only. Never edit any `slice.md`, the global `rules/*` library, code, or any file. Your deliverable is the verdict.
